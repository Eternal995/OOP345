...
namespace sdds {
    ...
    void TennisLog::addMatch(const TennisMatch& match) {
        ...
        for (size_t i = 0; i < m_count; i++) {
            temp[i].m_id = m_match[i].m_id;
            temp[i].m_name = m_match[i].m_name;
            temp[i].m_matchID = m_match[i].m_matchID;
            temp[i].m_winner = m_match[i].m_winner;
            temp[i].m_loser = m_match[i].m_loser;
        }
        temp[m_count].m_id = match.m_id;
        temp[m_count].m_name = match.m_name;
        temp[m_count].m_matchID = match.m_matchID;
        temp[m_count].m_winner = match.m_winner;
        temp[m_count].m_loser = match.m_loser;
        delete[] m_match;
        m_match = temp;
        m_count++;
    }
    ...
    TennisLog& TennisLog::findMatches(std::string name) {
        TennisLog* result = new TennisLog{};
        for (size_t i = 0; i < m_count; i++) {
    ...
    TennisMatch& TennisLog::operator[](size_t index) {
        if (index >= m_count) {
            TennisMatch* empty = new TennisMatch{};
            return *empty;
        }
